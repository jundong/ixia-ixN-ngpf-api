...
#          None
#==================================================================
proc saveconfig { configFile } {
    puts "Save configuration file into $configFile"
    set  result [ixNet exec saveConfig [ixNet writeTo $configFile -overwrite]]
}
...
proc Login { { location "localhost/8009"} { force 0 } { filename null } } {
    ...
    if { $location == "" } {
        # Even if we have default value,
        # but still to check whether User passed empty location value here
        set loginInfo "localhost/8009"
    }
    puts "Login...$loginInfo"
    
    set portInfo [ split $loginInfo "/" ]
    set remote_server     [ lindex $portInfo 0 ]
    if { [ llength $portInfo ] == 1 } {
        # If no Server port configuration case
        set portInfo 8009
    } else {
        # Multiple Server ports confgiration case
...
proc ConfigAttr { obj key value { step 0 } { port_step 0 } } {
    ...
    if { [regexp "ixNet::OBJ-" $mv] } {
        if { $step != 0 || $port_step != 0} {
            ...
            if { $port_step != 0 } {
                set nest [ixNet getL $mv nest]
                ixNet setM $nest \
                    -enabled true \
                    -step $port_step
            }
        } else {
            ixNet setA $mv -pattern singleValue
            ixNet commit
            ixNet setA $mv/singleValue -value $value
        }
    } else {
        ixNet setA $obj $key $value
    }
